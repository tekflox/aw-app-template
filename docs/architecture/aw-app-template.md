...
source: generated
edited: false
checksum: sha256:2259a3b2150cf9ba71866c3bb419964597c1e758b9a8457e4616f6e93ede5e51
---
# App Template
...
TEMPLATE — the fastest way to start a new aw-workspace app. Install it and you have a working app on day one: a `template` CLI that prints a configurable greeting, its own window in the Apps grid, and an HTTP + WebSocket backend — with tests and marketplace release already wired. The scaffold names its app, CLI, route and Python package after itself, so renaming every `template` occurrence to your app's name is the whole rename; see README.md.
...
- Then todo path declarado é relativo (/hello, /ws/echo) e os dois modos expõem /api/apps/aw-app-template/... idêntico, então código cliente e documentação usam uma forma só — se o standalone montasse com prefixo próprio, todo exemplo do template estaria certo num modo e errado no outro, e quem gera um app novo herdaria a divergência sem perceber. O WebSocket in-process fica em /ws/&lt;nome&gt; dentro do sub-app: a raiz /ws/* é reservada para sockets de controle do core
...
- Then a API responde em /api/apps/&lt;slug&gt;/hello de qualquer jeito, e a asserção sobre o HTML servido na raiz só entra em cena quando ui/dist/ existe de fato (test_standalone_serves_ui_dist_when_built:29) — um template cujo teste exige build quebra no primeiro clone, que é exatamente o momento em que ele precisa funcionar. O custo é honesto e vale nomear: num checkout sem build a cobertura da montagem estática é zero e o teste passa mesmo assim, com um return silencioso
- intended_status: `not_implemented` · derived health: `not_implemented`
